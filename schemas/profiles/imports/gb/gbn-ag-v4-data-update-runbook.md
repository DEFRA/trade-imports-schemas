...
which is gitignored. The committed baseline in `scripts/gbn-ag-data-import/data/`
is the only data that ships.
...
### 7. Update the PIMS mapping (human)

For any new or changed field, update its section in
`schemas/profiles/imports/gb/pims-data-mapping.md` directly (matching the field
to its PIMS counterpart is a judgement call). The document is maintained in
place; the earlier answers-file generator was retired when the PIMS
reconciliation concluded.
...
Then:
  ...
  4. For any new or changed field, update its section in
     schemas/profiles/imports/gb/pims-data-mapping.md directly.
...
- the edits and the PIMS mapping update (steps 5 and 7) - reviewed before trust;
