...
| `carrier` | `TradeParty` | yes | The person or organisation responsible for moving the goods from the consignment's despatch point to the delivery point. Pre-notifications distinguish commercial from private transporters via the `operator_activity_type` code on `partyTypeCode`; private transporters carry a UK transporter approval number (issued under the welfare-in-transport regime) as the value of `identifier`. `partyTypeCode` is `oneOf` a single string or an array of strings: a single string for the common case where the party carries one classification; an array when the party carries codes from more than one TRACES list (e.g. an `operator_activity_type` code plus a `classification_section_code`). Transporter status (active, suspended, expired) is not in the notification payload; consumers resolve it by looking the approval number up against the issuing scheme at consumption time. |
...
| `includedTradeLineItem` | array of `TradeLineItem` | yes (at least 1) | GBN-AG trade line item. One line per species or commodity on the certificate. Each line carries the species or commodity, the line quantity (head count for animals; weight for some germinals), and any notes about that line. Mixed consignments (for example cattle plus sheep, or live animals plus germinals) split across multiple lines, one per species or commodity. |
...
| Property | Type | Required | Description |
|---|---|---|---|
| `identifier` | array of object | no | A per-animal identifier. `typeCode` names the kind of identifier as a bare string; `urlId` names the scheme the typeCode is drawn from. Established typeCodes are EAR_TAG, PASSPORT, MICROCHIP, TATTOO, LEG_RING. For commodities without a specific identifier type, IDENTIFICATION_DETAILS carries a free-text identifier and DESCRIPTION carries a free-text animal description; these typeCodes are pending addition to the `animal_identifier_types` scheme. `content` is the identifier value itself - it is not drawn from a central registry (an ear-tag is issued by the farm; a microchip number is unique to the chip). Reference data: `https://refdata.tbc.defra.gov.uk/animal_identifier_types`. |
