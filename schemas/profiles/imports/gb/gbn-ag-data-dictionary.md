...
| `carrier` | `TradeParty` | yes | The person or organisation responsible for moving the goods from the consignment's despatch point to the delivery point. The party's role is carried on `partyRoleCode` as `CA` (Carrier, UNTDID 3035). Pre-notifications distinguish commercial from private transporters via the `operator_activity_type` code on `partyTypeCode`. `partyTypeCode` is an array of coded values: a single entry for the common case where the party carries one classification; multiple entries when the party carries codes from more than one TRACES list (e.g. an `operator_activity_type` code plus a `classification_section_code`). `identifier` is an optional party identifier, with `urlId` naming the register it is drawn from. |
